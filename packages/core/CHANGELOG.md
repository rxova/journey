...
- 15c9822: This release includes two core changes.
...
- 2551cd3: Remove the exported Journey runtime constants from `@rxova/journey-core` and simplify the related public types.
...
- d0ddbdf: Refactor the journey machine into smaller internal runtime, async-state, navigation, send, and control modules so the implementation is easier to read and maintain while preserving the existing public API and runtime behavior.
