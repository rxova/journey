...
/** Derives the full event type union from a user-supplied event union. */
export type JourneyFullEventType<TEvents extends JourneyBaseEvent> =
  | TEvents["type"]
...
export type ResolveJourneyTypes<T extends JourneyTypesInput> = {
  ...
  events: T extends { events: infer E extends JourneyBaseEvent } ? E : never;
...
type JourneyCustomSendEventType<TEvents extends JourneyBaseEvent> = Exclude<
  TEvents["type"],
  JourneyDefaultEventType
>;
...
/** Extracts the full event member for a given event type from the event union. */
export type JourneyEventFor<TEvents extends JourneyBaseEvent, TType extends string> = Extract<
  TEvents,
  { type: TType }
>;

/**
 * Resolves the payload type for a specific event type from the provided event
 * union. Falls back to `unknown` for built-in events that the union does not
 * override (and for event names not present in the union).
 */
export type JourneyPayloadFor<TEvents extends JourneyBaseEvent, TType extends string> = [
  JourneyEventFor<TEvents, TType>
] extends [never]
  ? unknown
  : JourneyEventFor<TEvents, TType> extends { payload: infer TPayload }
    ? TPayload
    : JourneyEventFor<TEvents, TType> extends { payload?: infer TPayload }
      ? TPayload
      : never;
...
/**
 * Built-in send events supported by machine convenience APIs. The payload is
 * resolved from the user union when a built-in's `type` is overridden, and is
 * always optional so convenience APIs (`goToNextStep()` etc.) can omit it.
 */
...
> =
  ...
  | {
      [TType in JourneyBuiltInSendEventType]: {
        type: TType;
        payload?: JourneyPayloadFor<TEvents, TType>;
      };
    }[JourneyBuiltInSendEventType];

/** Custom send events derived from a user-supplied event union (built-ins excluded). */
export type JourneyCustomSendEvent<TEvents extends JourneyBaseEvent = never> = {
  [TType in JourneyCustomSendEventType<TEvents>]: JourneyEventFor<TEvents, TType>;
}[JourneyCustomSendEventType<TEvents>];

/** Event union accepted by `JourneyMachine.send`. */
export type JourneySendEvent<TStepId extends string, TEvents extends JourneyBaseEvent = never> =
  | JourneyBuiltInSendEvent<TStepId, TEvents>
  | JourneyCustomSendEvent<TEvents>;

/** Event union available to transitions and guards for the declared event type set. */
export type JourneyEvent<
  TStepId extends string,
  TEvents extends JourneyBaseEvent = never
> = JourneySendEvent<TStepId, TEvents>;
...
> =
  ...
  | {
      id: TStepId;
      meta?: TStepMeta;
      onEnter?: JourneyStepLifecycleCallback<TContext, TStepId, never, THandlers>;
      onLeave?: JourneyStepLifecycleCallback<TContext, TStepId, never, THandlers>;
      effect?: JourneyStepEffect<TContext, TStepId, THandlers>;
      after?: Record<number, JourneyAfterTransition<TContext, TStepId>>;
    };
...
> = {
  ...
  context: TContext;
  handlers?: THandlers;
  steps: Record<TStepId, JourneyStepDefinition<TContext, TStepId, never, TStepMeta, THandlers>>;
};
...
> = Omit<JourneyDefinitionBase<TContext, TStepId, TStepMeta, THandlers>, "steps"> & {
  ...
  transitions?: JourneyTransitionsDefinition<TContext, TStepId, TEvents, THandlers>;
};
...
> = Required<Pick<JourneyDefinitionBase<TContext, TStepId, TStepMeta, THandlers>, "initial">> &
  Omit<JourneyDefinition<TContext, TStepId, TEvents, TStepMeta, THandlers>, "transitions"> & {
