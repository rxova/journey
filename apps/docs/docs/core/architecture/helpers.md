...
- Read [Send Pipeline](/docs/core/architecture/send) for the main consumer of `selectTransition(...)`,
  `resolveTransitionTarget(...)`, and abort/timeout helpers.
- Read [Navigation Commits](/docs/core/architecture/navigation) for the main consumer of
  `buildSnapshot(...)` and `transitionSnapshot(...)`.
- Read [Snapshot](/docs/core/snapshot), [Async](/docs/core/async), and [History](/docs/core/history) for the
