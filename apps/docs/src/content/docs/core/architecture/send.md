...
- It does not own the queue. That belongs to [Runtime Queue](./runtime.md).
- It does not build the committed snapshot shape. That belongs to [Navigation Commits](./navigation.md)
  and [Helpers](./helpers.md).
- It does not store async state itself. That belongs to [Async State](./async-state.md).

This separation is what keeps transition selection, snapshot commits, and async bookkeeping independently readable.

## Recommended Reading

- Read [Helpers](./helpers.md) for `selectTransition(...)`, abort/timeout handling, and target
  resolution.
- Read [Navigation Commits](./navigation.md) for the snapshot writes this file delegates to.
- Read [Core API Overview](../api/overview.md) and [Transitions Syntax](../api/transitions-syntax.md) for the
  public version of this behavior.
- Read [Core Async Behavior](../async.md) for the user-facing semantics of guard and context-update errors.
