...
Creating a machine starts it — `autoStart` defaults to `true`. The initial step is committed
...
function waitUntilSettled(machine: typeof checkout): Promise<void> {
  ...
  return new Promise((resolve) => {
    const stop = machine.subscriptions.subscribe(() => {
      if (!machine.getSnapshot().transition.pending) {
        stop();
        resolve();
      }
    });
...
Use `snapshot.transition.pending` as the default flag for disabling UI controls while navigation
...
snapshot.transition.pending; // false
...
```ts
const stopStepSubscription = checkout.subscriptions.subscribe(() =>
  renderStep(checkout.getSnapshot().currentStep?.id)
);
